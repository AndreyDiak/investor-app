...
const selectCharactersWithDifficulty = (filter: CharactersListFilter) =>
   createSelector(selectCharacters, (characters) =>
      filter === "all"
         ? characters.slice(0)
         : characters.filter((item) => item.difficulty === filter)
   );
...
export const selectCharactersWithMaxInARow = (filter: CharactersListFilter) =>
   createSelector(
      [selectCharactersWithDifficulty(filter), selectCurrent, selectMaxCardsInARow],
